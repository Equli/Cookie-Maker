...
class ConfiguratorRouter {
    ...
    selectBase = (req,res) => {
        ...
        if(!this.cmapp.data.COOKIE_BASES[baseName]) {
            return this.cmapp.showErrorPage(res, `There is not base like ${baseName}`);
        }
    ...
    addAddon = (req,res) => {
        ...
        if(!this.cmapp.data.COOKIE_ADDONS[addonName]) {
            return this.cmapp.showErrorPage(res, `There is not addon like ${addonName}`);
        }

        const addons = this.cmapp.getAddonsFromReq(req);

        if(addons.includes(addonName)) {
            return this.cmapp.showErrorPage(res, `${addonName} is already on your cookie. Cannot add it twice`);
        }
    ...
    deleteAddon = (req,res) => {
        ...
        if (!oldAddons.includes(addonName)) {
            return this.cmapp.showErrorPage(res, `Cannot delete something that isn't already added to the cookie: ${addonName}`);
        }
